    ...
    <section id="portfolio" className="py-20">
      <AnimatedSection>
        <div className="flex flex-col items-start gap-y-4 sm:flex-row sm:justify-between sm:items-center mb-10">
          ...
          <button className="bg-brand-blue-500 text-white px-6 py-3 rounded-full font-semibold hover:bg-opacity-90 transition-all duration-300 transform hover:scale-105 hover:-translate-y-1 flex-shrink-0">
            See All
          </button>
        </div>
      </AnimatedSection>
      ...
      <AnimatedSection delay={150}>
        <div className="grid md:grid-cols-2 gap-8 mb-10">
            <img src={activeItem.imageUrl} alt={activeItem.title} className="rounded-2xl w-full h-full object-cover transition-all duration-300 ease-in-out hover:scale-105 hover:shadow-xl" />
            <img src={nextItem.imageUrl} alt={nextItem.title} className="rounded-2xl w-full h-full object-cover hidden md:block transition-all duration-300 ease-in-out hover:scale-105 hover:shadow-xl" />
        </div>
      </AnimatedSection>
      ...
      <div key={activeItem.id} className="animate-fade-in-up">
        <div className="text-center max-w-2xl mx-auto">
            <h3 className="text-3xl md:text-4xl font-bold flex items-center justify-center gap-4 text-brand-dark dark:text-white">
              ...
              <button 
                  onClick={goToNext} 
                  className="bg-brand-blue-500 text-white w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 hover:bg-brand-blue-600 transition-all duration-300 transform hover:scale-110"
