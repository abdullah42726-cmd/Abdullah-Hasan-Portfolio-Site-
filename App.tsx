...
const PortfolioDetailModal: React.FC<{ item: PortfolioItem | null; onClose: () => void; }> = ({ item, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-75 z-[100] flex items-center justify-center p-4" style={{ animation: 'fade-in-up 0.3s ease-out forwards' }}>
      <div ref={modalContentRef} className="bg-brand-dark-2 rounded-lg shadow-xl w-full max-w-5xl h-[90vh] flex flex-col overflow-hidden">
        <header className="p-4 border-b border-gray-700 flex justify-between items-center flex-shrink-0">
          <h3 className="text-xl font-bold text-white truncate pr-4">{item.title}</h3>
          <button 
            onClick={onClose} 
            className="text-gray-500 hover:text-white text-3xl font-bold leading-none w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-700 transition-colors"
            aria-label="Close"
          >
            &times;
          </button>
        </header>
        <div className="flex-grow bg-brand-dark">
          {item.liveUrl ? (
            <iframe src={item.liveUrl} title={item.title} className="w-full h-full border-0" />
          ) : (
            <div className="p-8 text-center text-gray-400 flex flex-col justify-center items-center h-full">
              <h4 className="text-2xl font-bold text-white mb-4">No Live Preview Available</h4>
